...
class LIDC_IDRI(Dataset):
    ...
    def __init__(self, dataset_location, transform=None):
        ...
        for key, value in data.items():
            label = []
            status = -1
            for i in range(len(value['masks'])):
                if np.max(value['masks'][i]) > 0 and len(label) < 4:
                    label.append(value['masks'][i])
                    status = 1
            if status == 1:
    ...
    def __getitem__(self, index):
        ...
        l = random.randint(0, len(self.labels[index]) - 1)
        label = self.labels[index][l].astype(float)
