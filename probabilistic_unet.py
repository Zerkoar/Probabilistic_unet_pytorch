...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels, num_filters, no_convs_per_block, initializers, padding=True, posterior=False):
        ...
        self.num_filters = num_filters  # [32, 64, 128, 192]
        ...
        # self.layers.apply(self.initializers)
...
class AxisAlignedConvGaussian(nn.Module):
    ...
    def __init__(self, input_channels, num_filters, no_convs_per_block, latent_dim, initializers, posterior=False):
        ...
        # self.bn = nn.BatchNorm2d(2 * self.latent_dim)
        self.show_img = 0
        self.show_seg = 0
        self.show_concat = 0
        self.show_enc = 0
        ...
        nn.init.normal_(self.conv_layer.weight, std=0.001)
    ...
    def forward(self, input, segm=None):
        ...
        # 修改
        # mu_log_sigma = self.bn(mu_log_sigma)
        # loc = torch.where(mu_log_sigma != mu_log_sigma)
        # mu_log_sigma[loc] = 0
        # *********************
...
class ProbabilisticUnet(nn.Module):
    ...
    def __init__(self, input_channels=1, num_classes=2, num_filters=[32, 64, 128, 192], latent_dim=6, no_convs_fcomb=4,
                 beta=10.0):
    ...
    def forward(self, patch, segm, training=True):
        ...
        为patch构建预先的潜在空间，并通过UNet运行patch，如果训练是真的，还可以构建后潜伏期
        """
        if training:
    ...
    def elbo(self, segm, analytic_kl=True, reconstruct_posterior_mean=False):
        ...
        # criterion = nn.BCEWithLogitsLoss(size_average=None, reduce=None, reduction='none')
        ...
        # self.reconstruction = torch.argmax(self.reconstruction, dim=1)
        # self.reconstruction = torch.unsqueeze(self.reconstruction, dim=1)

        # reconstruction_loss = criterion(input=self.reconstruction.type(torch.float), target=segm)
        reconstruction_loss = criterion(self.reconstruction, torch.squeeze(segm, dim=1).type(torch.long))

        self.reconstruction_loss = torch.sum(reconstruction_loss)
        self.mean_reconstruction_loss = torch.mean(reconstruction_loss)

        # return -(self.reconstruction_loss + self.beta * self.kl)
        return -(self.reconstruction_loss + self.beta * self.kl), self.reconstruction
